...
void hmac_sha256( const unsigned char *text,      /* pointer to data stream        */
...
*/
    ...
    hmac_sha256( message, strlen(message), key, strlen(key), digest1 );

    p = base64( digest, SHA256_DIGEST_LENGTH );
    p1 = base64( digest1, SHA256_DIGEST_LENGTH );
//    if ( p )
//	printf( "RESULT: %s\n", p );
//    { int c; fputs( ": ", stderr ); c = getchar(); }
    // RESULT: Nace+U3Az4OhN7tISqgs1vdLBHBEijWcBeCqL5xN9xg=

    int result = strcmp(digest, digest1);
    printf("비교값 : %d\n",result);
/*
